...
from cams_read import get_cams_air_quality
...
def load_cams_data():
    """Load CAMS Air Quality (Europe) and Atmospheric Composition (Global) data with smart caching and 10 UTC cutoff"""
    #automatically clean up old data files
    try:
        # --- Air Quality (Europe) data ---
        st.info("🌍 Loading CAMS Europe Air Quality data...")
        now_utc = datetime.utcnow()
        hour_utc = now_utc.hour
        today = datetime.utcnow()
        today_str = today.strftime("%Y-%m-%d")

        # Before 10 UTC: use latest available (old) data
        if hour_utc < 10:
            yesterday = datetime.utcnow() - timedelta(days=1)
            yesterday_str = yesterday.strftime("%Y-%m-%d")
            st.info("⏰ Before 10 UTC: using data from yesterday (no new download).")
            ds_europe = get_cams_air_quality(yesterday_str)
            date_europe = yesterday_str
        # After 10 UTC: try to get today's data, download if missing or forced
        else:
            st.success(f"⏰ After 10 UTC: download new data for today ({today_str}).")
            ds_europe = get_cams_air_quality(today_str)
            date_europe = today_str
        # Download new data if needed (after 10 UTC or forced)

        # Return both datasets and their dates
        return (ds_europe, date_europe)

    except Exception as e:
        st.error(f"❌ Error loading CAMS data: {str(e)}")
        return (None, None)
...
def main():
    ...
    if os.path.exists(data_folder):
        ...
        st.sidebar.info(f"📁 Data files: {len(files)}")
        st.sidebar.info(f"💾 Total size: {total_size / (1024*1024):.1f} MB")
